...
function generateCode() { //gera um codigo unico
    let text = '';
    const possible = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
    for (let i = 0; i < 5; i++)
        text += possible.charAt(Math.floor(Math.random() * possible.length)); //caracteres possiveis
    return text;
}

async function postLink(req: Request, res: Response) {
    const link = req.body as Link; //as - conversão
    ...
    link.id = result.id!;
    res.status(201).json(link); //devolve objeto
